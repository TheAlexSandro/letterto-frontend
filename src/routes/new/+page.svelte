<script lang="ts">
	...
	const handeSubmit = async (e: Event) => {
		...
		const ft = await fetch('/api/letters?path=new', {
			method: 'POST',
			body: fd
		});
				...
				<form method="post" onsubmit={handeSubmit}>
					...
					<div class="music-picker">
						...
						{#if selected}
							<div class="selected-info">
								<img src={selected.album.cover_small} alt={selected.title} />
								<div class="info">
									<span class="title"
					...
					<div class="btns">
						<button disabled={buttonLoad} class="submit" type="submit">
							...
							{:else}
								Create <i class="ri-heart-add-2-line"></i>
							{/if}</button
						>
					</div>
				</form>
			{:else}
				<section class="success-w">
					<div class="success">
						...
						<p class="sub">
							Successfully created a letter for: <strong>{recipientName}</strong>.
						</p>
